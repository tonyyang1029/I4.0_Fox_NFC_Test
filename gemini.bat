...
:: Start adb logcat in a new background process and get its PID
set "LOGCAT_PID="
for /f "tokens=2" %%i in ('adb -d logcat -b all ^> "%LOG_FILENAME%" 2^>^&1 ^& echo %errorlevel%') do set "LOGCAT_PID=%%i"
if not defined LOGCAT_PID (
    echo [%time%] ERROR: Failed to start logcat.
    goto :end
)
echo [%time%] Logcat started with PID: %LOGCAT_PID%
...
if defined LOGCAT_PID (
    taskkill /PID %LOGCAT_PID% /F >nul 2>nul
    echo [%time%] Stopped logcat process with PID: %LOGCAT_PID%.
) else (
    echo [%time%] Logcat PID not found, unable to stop gracefully.
)
